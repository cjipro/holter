"""Pulse Platform API — Surface 5 of Holter (the face of Pulse).

The **engine-only buyer path** made first-class. FastAPI app that emits
...
app = FastAPI(
    ...
    version="0.1.0",
    description=(
        "Evidentiary investigation engine API. v0 — read-side + lineage "
        "verification work; run + signals blocked on PULSE-93. See "
        "`~/.claude/plans/adaptive-mapping-popcorn.md` for the full plan."
    ),
)
